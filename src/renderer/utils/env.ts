...
export function log(level: LogLevel, message: string, ...args: any[]) {
  ...
  if (!config.debug.enabled && level === 'debug') {
    return
  }

  const logLevels: LogLevel[] = ['debug', 'info', 'warn', 'error']
  const currentLevelIndex = logLevels.indexOf(config.debug.logLevel as LogLevel)
 ...
 * 初始化渲染进程环境配置
 */
export function initializeRendererEnv(): void {
  const config = getAppConfig()

  log('info', 'Renderer environment initialized', {
    env: config.app.env,
    debug: config.debug.enabled,
    version: config.app.version
  })
